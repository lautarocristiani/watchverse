...
export default async function MovieDetailPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    const movie = await getMediaDetails(mediaType, id);
    ...
    const [mediaStatus, { reviews, average }, userReview, watchlistItems, watchedItems, ratingsMap] = await Promise.all([
        user ? getMediaStatus(supabase, user.id, movieId, mediaType) : null,
        getReviewsForMedia(supabase, movieId, mediaType),
        user ? getUserReviewForMedia(supabase, user.id, movieId, mediaType) : null,
    ...
    const similarMovies = movie.similar ? enrichMedia(movie.similar.results, watchlistIds, watchedIds, ratingsMap) : [];
    ...
    return (
        <div className="pb-16">
            <section className="relative h-[50vh] md:h-[60vh]">
                ...
                {movie.backdrop_path && (
                    <Image src={`${TMDB_IMAGE_BASE_URL}${movie.backdrop_path}`} alt={`Backdrop for ${movie.title}`} fill className="object-cover object-top" priority unoptimized />
                )}
            ...
            <div className='container mx-auto px-4'>
                <section className="flex flex-col md:flex-row gap-8 -mt-[25vh] z-10 relative">
                    <div className="w-full max-w-[200px] md:max-w-[250px] mx-auto md:mx-0 flex-shrink-0">
                        <Image
                            src={movie.poster_path ? `${TMDB_POSTER_URL}${movie.poster_path}` : '/placeholder-poster.svg'}
                    ...
                    <div className="w-full md:w-3/4 pt-4 md:pt-16 text-center md:text-left space-y-4">
                        ...
                        <div className="flex flex-wrap gap-x-4 gap-y-2 justify-center md:justify-start items-center">
                            <span className="text-text-secondary-light dark:text-text-secondary-dark">{movie.release_date?.substring(0, 4)}</span>
                        ...
                        <p className="text-text-secondary-light dark:text-text-secondary-dark leading-relaxed max-w-2xl mx-auto md:mx-0 pt-2">{movie.overview}</p>
                        <div className="pt-4 flex justify-center md:justify-start">
                            <MediaActionButtons mediaId={movie.id} mediaType={mediaType} initialStatus={mediaStatus} isLoggedIn={!!user} />
                        </div>
                    </div>
                </section>
                    ...
                    <section>
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 mt-4">
                            ...
                            <div className="order-1 lg:order-2">
                                {user ? <ReviewForm mediaId={movieId} mediaType={mediaType} userReview={userReview} /> : <p className="text-text-secondary-light dark:text-text-secondary-dark">You must be logged in to leave a review.</p>}
                            </div>
                        </div>
                    </section>
